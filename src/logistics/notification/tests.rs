...
#[actix_web::test]
async fn dispatch_created_records_customer_and_driver_and_prefers_email() {
...
#[actix_web::test]
async fn falls_back_to_sms_then_skips_when_no_contact() {
    let _db = TestDb::create();
    let org = org();
    ...
    let n = Notification::record_dispatch_created(org.id, Uuid::new_v4(), &phone_only, None).await.expect("rec");
    ...
    let n2 = Notification::record_dispatch_created(org.id, Uuid::new_v4(), &no_contact, None).await.expect("rec2");
    assert!(n2.iter().all(|n| n.status == NotificationStatus::Skipped));
}

#[actix_web::test]
async fn dispatch_delivered_notifies_only_the_customer() {
    let _db = TestDb::create();
    let org = org();
    ...
    let recorded = Notification::record_dispatch_delivered(org.id, dispatch_id, &customer).await.expect("rec");
...
#[actix_web::test]
async fn list_by_org_is_scoped_and_newest_first() {
    let _db = TestDb::create();
    ...
    Notification::record_dispatch_created(a.id, Uuid::new_v4(), &ca, None).await.expect("a1");
    Notification::record_dispatch_delivered(a.id, Uuid::new_v4(), &ca).await.expect("a2");
    Notification::record_dispatch_created(b.id, Uuid::new_v4(), &cb, None).await.expect("b1");
...
#[actix_web::test]
async fn deleting_the_org_cascades_to_its_notifications() {
    let _db = TestDb::create();
    let org = org();
    let c = Customer::create_customer(org.id, "Gone", "0").expect("c");
    let d = Uuid::new_v4();
    Notification::record_dispatch_created(org.id, d, &c, None).await.expect("rec");
...
async fn deliver_queued_best_effort_does_not_touch_a_skipped_notification() {
    ...
    let recorded = Notification::record_dispatch_created(org.id, dispatch_id, &no_contact, None).await.expect("record");
...
#[actix_web::test]
async fn record_dispatch_created_and_delivered_fall_back_to_the_fixed_template_without_an_api_key() {
    // SAFETY: this module is the only test file that reads ANTHROPIC_API_KEY
    // concurrently in this way — see ai/status.rs's equivalent test for the
    // full rationale.
    unsafe {
    ...
    let org = org();
    let mut customer = Customer::create_customer(org.id, "Asha Rao", "9 Market Rd").expect("customer");
    customer.set_contact(None, Some("asha@example.com".into())).expect("contact");
    let dispatch_id = Uuid::new_v4();
    let short_id = &dispatch_id.to_string()[..8];

    let created = Notification::record_dispatch_created(org.id, dispatch_id, &customer, Some("+91 98888 22222"))
        .await
        .expect("record created");
    let cust = created.iter().find(|n| n.recipient_kind == "customer").unwrap();
    assert_eq!(
        cust.body,
        format!("Hi Asha Rao, your order (ref {short_id}) has been dispatched and is on its way.")
    );
    let driver = created.iter().find(|n| n.recipient_kind == "driver").unwrap();
    assert_eq!(driver.body, format!("New trip assigned: dispatch {short_id} to Asha Rao."));

    let delivered = Notification::record_dispatch_delivered(org.id, dispatch_id, &customer)
        .await
        .expect("record delivered");
    assert_eq!(
        delivered[0].body,
        format!("Hi Asha Rao, your order (ref {short_id}) has been delivered. Thank you!")
    );
}

#[actix_web::test]
async fn recording_a_notification_indexes_it_for_the_assistant() {
    ...
    Notification::record_dispatch_created(org.id, dispatch_id, &customer, None).await.expect("record");
